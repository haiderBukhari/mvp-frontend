...
import BookingPipelineDemo from './BookingPipelineDemo';
...
const HeroSection = () => {
  ...
  return <section className="relative w-full py-12 md:py-20 px-6 md:px-12 flex flex-col items-center justify-center overflow-hidden bg-background">
      ...
      <div className={`relative z-10 max-w-6xl text-center space-y-6 transition-all duration-700 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="flex justify-center">
            <span className="inline-flex items-center gap-2 px-3 py-1 text-xs font-medium rounded-full bg-muted text-primary">
              <span className="flex h-2 w-2 rounded-full bg-primary"></span>
              Launching booking automation features
              <Loader className="h-3 w-3 animate-spin text-primary" />
            </span>
        </div>
        
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-medium tracking-tighter text-balance text-foreground">
          Automate Your Media Bookings with <span className="text-foreground">Human-in-the-Loop</span> Control
        </h1>
        
        <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto text-balance">
          From booking submission to PDF generation, live chat, and real-time confirmation—manage everything in one powerful dashboard. Built for modern media companies who value automation and efficiency.
        </p>
          ...
          <div className="bg-card backdrop-blur-md w-full">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center gap-4">
                ...
                <span className="text-foreground font-medium">Media Booking Pipeline</span>
              </div>
